		...
		<div id="layoutSidenav_content">
			<main>
				...
				<center>
					...
					<form action="updateNotice.do" id="viewForm" method="post"
						...
						<table border="1" cellpadding="0" cellspacing="0">
							<c:choose>
								<c:when test="${sessionScope.level eq 'admin'}">
									<tr>
										<td>제목</td>
										<td align="left"><input name="ntitle" type="text"
											value="${notice.ntitle}" size="52">
										</textarea></td>
									</tr>
									<tr>
										<td>내용</td>
										<td align="left"><textarea name="ncontent" cols="40"
												rows="5" style="resize: none;">${notice.ncontent}</textarea></td>
									</tr>
								</c:when>
								<c:otherwise>
									<tr>
										<td>제목</td>
										<td align="left"><input name="ntitle" type="text"
											value="${notice.ntitle}" size="52" readonly></td>
									</tr>
									<tr>
										<td>내용</td>
										<td align="left"><textarea name="ncontent" cols="40"
												rows="5" readonly style="resize: none;">${notice.ncontent}</textarea></td>
									</tr>
								</c:otherwise>
							</c:choose>
							...
							<tr>
								<c:if test="${notice.filename ne null}">
									<tr>
										<th>다운로드</th>
										<td><a href="fileDownload.do?filename=${notice.filename}">
												<input type="text" id="filename" value="${notice.filename}"
												name="filename" class="form-control" readonly="readonly" />
										</a> <c:if test="${sessionScope.level eq 'admin' }">
												<button id="filedelete" type="button" class="btn_previous"
													style="float: right">파일삭제</button>
											</c:if>
									</tr>
								</c:if>
