...
<script type="text/javaScript" language="javascript" defer="defer">
	function fn_egov_link_page(pageNo) {
		location.href = "/TeamPl/ProductList.do?viewPage=" + pageNo;
	}
</script>
						...
						<c:forEach var="product" items="${productList}" begin="0" end="3">
							<td align="center">
								<div class="image_wrap"
									data-nno="${imageList[status.index].nno}"
						...
						<c:forEach var="product" items="${productList}" begin="4" end="7">
							<td align="center">
								<div class="image_wrap"
									...
									data-uuid="${imageList[status.index].uuid}"
									data-filename="${imageList[status.index].fileName}">
									<a href="selectProduct.do?nno=${product.pno }"><img
										width="120" height="200"></a>
								</div> ${product.pname }<br>
							</td>
						</c:forEach>
					...
					<tr>
						<c:choose>
							<c:when test="${sessionScope.level eq 'admin'}">
								<td align="right"><button type="button" onclick="location.href='insertProduct.do'" class="btn btn-primary">상품 입력</button>
							</c:when>
							<c:otherwise>
							</c:otherwise>
						</c:choose>
