	...
	<div id="layoutSidenav">
		...
		<div id="layoutSidenav_content">
			<main>
				<div class="card mb-4">
					<div class="card-body">
						<spring:eval expression="sessionScope.id" var="loggedInUserId" />

						<c:url var="loginUrl" value="/loginWrite.do" />
						<c:url var="logoutUrl" value="/logout.do" />
						<c:url var="registerUrl" value="/insertMember.do" />
						<c:url var="boardUrl" value="/ComplList.do" />
						<c:choose>
							<c:when test="${not empty loggedInUserId}">
								<div class="myPage">
									<div class="1">
										<a href="MyCompl.do?id=${sessionScope.id}">내 건의보기</a>
									</div>
									<br>
									<div class="2">
										<a href="selectMember.do?uid=${sessionScope.id}">회원수정하기</a>
									</div>
									<br>
									<div class="3">
										<a href="deleteMember.do?id=${sessionScope.id}">회원탈퇴하기</a>
									</div>
									<br>
									<c:if test="${sessionScope.level eq 'admin' }">
										<a href="ListMember.do">회원관리</a>
									</c:if>
								</div>
							</c:when>
							<c:otherwise>
