...
<script type="text/javaScript" language="javascript" defer="defer">
	function fn_egov_link_page(pageNo) {
		location.href = "/TeamPl/MenuList.do?viewPage=" + pageNo;
	}
</script>
...
<script type="text/javascript">
	$(document)
			.ready(
					function() {
						var hiddenimg = document.querySelectorAll(".imgli");
						var hiddenimgList = [];
		...
		<div id="layoutSidenav_content">
			<main>
				...
				<form:form commandName="searchVO" id="listForm" name="listForm" method="post">
					<table>
							<tr>
								<c:forEach var="menu" items="${menuList}" begin="0" end="3">
									<td>
										<div class="image_wrap" data-nno="${imageList[status.index].mno}"
											data-path="${imageList[status.index].uploadPath}"
											data-uuid="${imageList[status.index].uuid}"
											data-filename="${imageList[status.index].fileName}">
											<a href="selectMenu.do?mno=${menu.mno }"><img width="120"
												height="200"></a>
										</div> ${menu.myear}년  ${menu.month}월
									</td>		
								</c:forEach>
							</tr>
			
						<c:forEach var="menu" items="${menuList}" varStatus="status">
							<input type="hidden" class="imgli"
								data-value="${imageList[status.index]}">
						</c:forEach>
					</table>
				...
				<c:choose>
					<c:when test="${sessionScope.level eq 'admin'}">
						<a href="insertMenu.do" class="button">메뉴 입력</a>
					</c:when>
					<c:otherwise>
